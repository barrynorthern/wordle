...
let state = {};
let makeGuess=(a,f)=>(s=>!s.g?(s.g="salet"):
(n=>((m,h,j)=>(s.s=s.s??h.map(()=>({b:n()})),s.r=s.r??n(),
h.map((c,i)=>f[i]===1&&s.r.add(c)&&m.add(c)),
h.map((c,i)=>!s.s[i].f&&(
f[i]==0&&(s.s[i].f=c,s.r.add(c)),
f[i]==1&&s.s[i].b.add(c),
f[i]==2&&h.map((_,j)=>(!s.s[j].f&&(!m.has(c)||i===j))&&s.s[j].b.add(c)))),
(r=>s.g=a.find(x=>r.test(x)))
(new RegExp(`^${j([...s.r].map(c=>`(?=.*${c})`))}${j(s.s.map(p=>p.f?p.f:p.b.size?`[^${j([...p.b])}]`:"."))}$`))))
(n(),[...s.g],x=>x.join("")))(_=>new Set()))(state)
